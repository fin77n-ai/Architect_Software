...
def generate_architecture_md(directory, nodes, edges, category_map, file_info_dict, run_command=None):
    ...
    run_cmd = ' '.join(run_command) if run_command else '（未检测到）'

    md_content = "# 项目架构全景图 (Auto-Generated by Architect)\n\n"
    md_content += "> 由 `architect map` 自动生成。把这个文件分享给 Claude (Sonnet) 即可获得完整项目背景。\n\n"
    md_content += f"**运行命令:** `{run_cmd}`\n\n"

    # 协作角色声明
    md_content += "---\n\n"
    md_content += "## 🤝 AI 协作角色分工\n\n"
    md_content += "| 角色 | 模型 | 职责 |\n"
    md_content += "|------|------|------|\n"
    md_content += "| **规划者** | Claude Sonnet | 理解需求、生成 brief、验收结果 |\n"
    md_content += "| **执行者** | Gemini / Haiku | 按 brief 写代码、完成后填 handoff |\n\n"
    md_content += "**执行者规则:**\n"
    md_content += "1. 改动前说明要改哪个文件和原因\n"
    md_content += "2. 不执行任何 git 命令\n"
    md_content += "3. 不修改 brief 中 🚫 列表内的文件\n"
    md_content += "4. 完成后填写 handoff 模板\n\n"
    md_content += "**规划者工作流:**\n"
    md_content += "```bash\n"
    md_content += "architect brief --module [层]   # 生成给执行者的任务 brief\n"
    md_content += "architect diff                  # 验证执行者没有越界改动结构\n"
    md_content += "architect doctor                # 验证程序可正常运行\n"
    md_content += "```\n\n"
    md_content += "---\n\n"
...
def scan_project(directory, run_command=None):
    ...
    generate_architecture_md(directory, nodes, unique_edges, category_map, file_info_dict, run_command)
